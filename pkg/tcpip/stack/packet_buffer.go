...
func NewPacketBuffer(opts PacketBufferOptions) *PacketBuffer {
	...
	if opts.ReserveHeaderBytes != 0 {
		v := bufferv2.NewView(opts.ReserveHeaderBytes)
		v.Grow(opts.ReserveHeaderBytes)
		pk.buf.Append(v)
...
// ToBuffer returns an owned copy of the underlying storage of the whole packet.
func (pk *PacketBuffer) ToBuffer() bufferv2.Buffer {
	b := pk.buf.Clone()
	b.TrimFront(int64(pk.headerOffset()))
	return b
}

// ToView returns an owned copy of the underlying storage of the whole packet
// as a view.
func (pk *PacketBuffer) ToView() *bufferv2.View {
...
func (pk *PacketBuffer) headerView(typ headerType) bufferv2.View {
	h := &pk.headers[typ]
	if h.length == 0 {
...
// View returns an owned copy of the underlying storage of h as a *bufferv2.View.
...
// ToSlice returns a owned copy of data in r.
...
// ToView returns a owned copy of data in r.
...
// BufferSince returns packet payload starting from and including a particular
// header.
//
// The returned View is owned by the caller - its backing buffer is separate
// from the packet header's underlying packet buffer.
