...
func (e *endpoint) processExtensionHeaders(h header.IPv6, pkt *stack.PacketBuffer, forwarding bool) error {
	...
	for {
		...
		case header.IPv6FragmentExtHdr:
			...
			if resPktToDecRef, err = e.processFragmentExtHdr(&extHdr, &it, &pkt, h); err != nil {
				return err
			}
...
func (e *endpoint) processFragmentExtHdr(extHdr *header.IPv6FragmentExtHdr, it *header.IPv6PayloadIterator, pkt **stack.PacketBuffer, h header.IPv6) (*stack.PacketBuffer, error) {
	stats := e.stats.ip
	...
	if extHdr.FragmentOffset() == 0 {
		...
		default:
			stats.MalformedPacketsReceived.Increment()
			stats.MalformedFragmentsReceived.Increment()
			return nil, fmt.Errorf("known extension header = %#v present after fragment header in a non-initial fragment", lastHdr)
		}
	}
	...
	if fragmentPayloadLen == 0 {
		...
		stats.MalformedPacketsReceived.Increment()
		stats.MalformedFragmentsReceived.Increment()
		return nil, fmt.Errorf("fragment has no payload")
	}
	...
	if extHdr.More() && fragmentPayloadLen%header.IPv6FragmentExtHdrFragmentOffsetBytesPerUnit != 0 {
		...
		return nil, fmt.Errorf("found fragment length = %d that is not a multiple of 8 octets", fragmentPayloadLen)
	}
	...
	if lengthAfterReassembly > header.IPv6MaximumPayloadSize {
		...
		return nil, fmt.Errorf("determined that reassembled packet length = %d would exceed allowed length = %d", lengthAfterReassembly, header.IPv6MaximumPayloadSize)
	}
